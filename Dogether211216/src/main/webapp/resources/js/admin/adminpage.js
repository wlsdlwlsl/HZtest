	...
	function adminMemberList(){
		$.ajax({
			...
			success : function(resultMember){
				// ##### 동적으로 런닝구방 리스트 만들기 #####
				var adminMemberList = $("#adminMemberList");					// admin.jsp에 table id를 변수에 저장
				adminMemberList.empty();									// 비워놓고 시작 ==> 기존 리스트가 있을 수 있으니까
				adminMemberList.append(
					"<tr>"
					//+ "<th width='200'>프로필사진</th>"
					+ "<th width='100'>ID</th>"
					...
					+ "<th width='100'>삭제</th>"
					+"</tr>");
				for(row of resultMember){									// 향상된 for문 (list row : resultRNG) ==> 변수명은 상관없음
					console.log(row);									// 데이터가 잘 넘어왔는지 확인
					var tr = $("<tr/>");								// <tr/> 객체 생성
...
//					tr.append(member_realfname);
					var memberID = $("<td id='memberID' width='100' />").html(row.memberID);		// td객체를 생성 ==> roomNumber를 td에 담는다
					tr.append(memberID); 													// tr에 roomNumber를 담은 td를 추가
					...
					tr.append(userName);
					var phoneNumber = $("<td width='200' />").html(row.phoneNumber);		// td객체를 생성 ==> roomNumber를 td에 담는다
					tr.append(phoneNumber); 													// tr에 roomNumber를 담은 td를 추가
	 ...
	 $('#adminMember').click(function(evt){
		 ...
		 $("#adminMemberList").show();		// 런닝구 리스트를 안보임에서 보임으로 변경
//		 $(".sortRNG").css("display","none");	// 왼쪽 상단에 sort 콤보박스 안보이게 css 변경
//		 $(".sortBST").css("display","none");	// 왼쪽 상단에 sort 콤보박스 보이게 css 변경
//		 $(".sortBST").css("display","none");	// 왼쪽 상단에 sort 콤보박스 보이게 css 변경
